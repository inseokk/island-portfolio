...
import { AVATAR_TARGETS } from "../data/islands";
...
export function useAvatar() {
  ...
  const walkTo = useCallback((islandId) => {
    if (!avatarRef.current) return;
    if (activeIsland === islandId) return; // already there
    ...
    // Calculate duration based on distance so speed feels consistent
